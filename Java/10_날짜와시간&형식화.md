...
Calendar는 추상클래스이기 때문에 직접 객체를 생성할 수 없다. Calendar를 상속받아 완전히 구현한 클래스로는 GregorianCalendar와 BuddhistCalendar가 있고, getInstance()를 통해서 완전히 구현된 클래스의 인스턴스를 얻어야 한다.   
